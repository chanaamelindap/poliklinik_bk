...
<head>
    ...
    <style>
        body {
            background: linear-gradient(to right, #EAF4E3, #FFFFFF);
        }
    </style>
</head>

<body class="flex gap-5 bg-[#EAF4E3]">
    <!-- Side Bar -->
    <?= include("../../components/sidebar_pasien.php"); ?>
    <!-- Side Bar End -->

    <main class="flex flex-col w-full bg-white pb-10 rounded-lg shadow-lg">
         <header class="flex items-center gap-3 px-8 py-7 bg-[#A3D9A5] rounded-t-lg shadow-lg">
            <img src="../../assets/icons/notebook-pen.svg" alt="" width="30px" class="invert">
            <h1 class="text-3xl font-medium text-[#388E3C]">Riwayat Daftar Poli</h1>
        </header>

        <article class="mx-8 mt-8 p-8 bg-white shadow-lg rounded-lg">
            <div class="flex flex-col gap-3">
                <h1 class="bg-[#EAF4E3] px-5 py-4 text-[#2E7D32] text-xl font-medium rounded-t-2xl">Riwayat Daftar Poli</h1>
                <table class="w-full table-fixed border border-green-500">
                       <thead>
                           <tr>
                          ...
                          <tbody>
                            <?php
                            $data_poli = query("SELECT p.no_rm, p.nama AS nama_pasien, po.nama_poli, d.nama AS nama_dokter, dp.keluhan FROM daftar_poli dp JOIN pasien p ON dp.id_pasien = p.id JOIN jadwal_periksa jp ON dp.id_jadwal = jp.id JOIN dokter d ON jp.id_dokter = d.id JOIN poli po ON d.id_poli = po.id WHERE dp.id_pasien = '$_SESSION[id]'");
                            $no = 1;
